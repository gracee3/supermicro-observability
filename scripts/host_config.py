...
ENV_FILE = Path(os.environ.get("OBSERVABILITY_CONFIG_FILE", PROJECT / ".env"))
PROM_TEMPLATE = PROJECT / "prometheus" / "prometheus.yml.in"
RUNTIME = Path(os.environ.get("OBSERVABILITY_RUNTIME_DIR", PROJECT / "runtime"))
PROM_RUNTIME = RUNTIME / "prometheus"
FILE_SD = PROM_RUNTIME / "file_sd"
LOCAL_TEXTFILE = RUNTIME / "textfile_collector"
PASSWORD_FILE = Path(
    os.environ.get(
        "GRAFANA_PASSWORD_FILE", RUNTIME / "secrets" / "grafana-admin-password"
    )
)
...
def defaults(existing: dict[str, str]) -> dict[str, str]:
    password = existing.get("GRAFANA_ADMIN_PASSWORD", "") or read_password()
...
def validate_scalar_config(config: dict[str, str]) -> None:
    ...
    if len(password) < 32 or any(character in password for character in "\n\r\0"):
        raise ConfigError("Grafana admin password must contain at least 32 safe characters")
    try:
...
def write_env(config: dict[str, str]) -> None:
    ...
    ordered = CONFIG_ORDER + sorted(
        set(config) - set(CONFIG_ORDER) - {"GRAFANA_ADMIN_PASSWORD"}
    )
    lines = [
        "# Generated by scripts/configure-host. Edit by rerunning that command.",
        "# This file contains persistent host identifiers; never commit it.",
    ]
    for key in ordered:
...
def env_content(config: dict[str, str], show_sensitive: bool) -> str:
    rows = []
    for key in ["GRAFANA_ADMIN_PASSWORD", *CONFIG_ORDER]:
        value = config[key]
...
def configure(args: argparse.Namespace) -> int:
    existing = read_env()
    password_was_present = bool(
        existing.get("GRAFANA_ADMIN_PASSWORD", "") or read_password()
    )
...
def check_config(quiet: bool = False, compose: bool = True) -> int:
    if not ENV_FILE.exists():
        raise ConfigError(f"configuration file is missing: {ENV_FILE}")
    if stat.S_IMODE(ENV_FILE.stat().st_mode) not in {0o600, 0o640}:
        raise ConfigError(f"configuration file must have mode 0600 or 0640: {ENV_FILE}")
    if not PASSWORD_FILE.exists():
        raise ConfigError(f"Grafana password file is missing: {PASSWORD_FILE}")
    if stat.S_IMODE(PASSWORD_FILE.stat().st_mode) not in {0o600, 0o640}:
        raise ConfigError(
            f"Grafana password file must have mode 0600 or 0640: {PASSWORD_FILE}"
        )
